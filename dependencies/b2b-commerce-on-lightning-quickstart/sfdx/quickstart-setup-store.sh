...
then
	...
	echo_attention "Community Network Name ${communityNetworkName}"
	...
	echo_attention "Community Site Name ${communitySiteName}"
	...
	echo_attention "Community Experience Bundle Name ${communityExperienceBundleName}"
	...
	echo "Retrieve the org namespace..."
	orgNamespace=$(sfdx force:org:display --json | sed -n 's|.*"namespace": "\([^"]*\)".*|\1|p')
	flowOrgNamespace=''
	if [ -z "$orgNamespace" ]
  then
    echo "The org does not has a namespace - the process will use default."
  else
	  flowOrgNamespace=$orgNamespace"__"
  fi
	...
	function register_and_map_credit_card_payment_integration {
		...
		apexClassId=`sfdx force:data:soql:query -q "SELECT Id FROM ApexClass WHERE Name='DRB2B_PaymentGatewayAdapter' LIMIT 1" -r csv |tail -n +2`
		echo "Creating PaymentGatewayProvider record using ApexAdapterId=$apexClassId."
		sfdx force:data:record:create -s PaymentGatewayProvider -v "DeveloperName='DR_PaymentGateway' ApexAdapterId='$apexClassId' MasterLabel='DR Payment Gateway' IdempotencySupported='Yes' Comments='Digital River Payment Gateway Provider'"

		# Creating Payment Gateway
		paymentGatewayProviderId=`sfdx force:data:soql:query -q "SELECT Id FROM PaymentGatewayProvider WHERE DeveloperName='DR_PaymentGateway' LIMIT 1" -r csv | tail -n +2`
		namedCredentialId=`sfdx force:data:soql:query -q "SELECT Id FROM NamedCredential WHERE MasterLabel='Digital River API' LIMIT 1" -r csv | tail -n +2`
		...
		sfdx force:data:record:create -s PaymentGateway -v "MerchantCredentialId=$namedCredentialId PaymentGatewayName='Digital River Payment Gateway' PaymentGatewayProviderId=$paymentGatewayProviderId Status=Active"
		...
		paymentGatewayId=`sfdx force:data:soql:query -q "SELECT Id FROM PaymentGateway WHERE PaymentGatewayName='Digital River Payment Gateway' LIMIT 1" -r csv | tail -n +2`
...
function register_and_map_Sync_credit_card_payment_integration {
	echo "Registering credit card payment integration."

	# Creating Store Integrated Service
		syncstoreId=`sfdx force:data:soql:query -q "SELECT Id FROM WebStore WHERE Name='$communityNetworkName' LIMIT 1" -r csv | tail -n +2`
		...
		echo "Creating StoreIntegratedService using the $communityNetworkName store and Integration=$paymentGatewayId (PaymentGatewayId)"
		sfdx force:data:record:create -s StoreIntegratedService -v "Integration='$paymentGatewayId' StoreId='$syncstoreId' ServiceProviderType=Payment"
}
echo "$1"
echo "$1" == "SyncStore"
		if [[ "$1" == *SyncStore ]]
			then
					register_and_map_Sync_credit_card_payment_integration
				else
					...
					register_and_map_integration "DRB2B_ShippingSample" "COMPUTE_SHIPPING" "Shipment"
					register_and_map_integration "DRB2B_CartTaxCalculations" "COMPUTE_TAXES" "Tax"
	...
	sed "s/sfdc_checkout__CheckoutTemplate/$flowOrgNamespace$mainFlowName/g" $checkoutMetaFile > $tmpfile
	...
	username=`sfdx force:user:display | grep "Id" | sed 's/Id//g;s/^[[:space:]]*//g'`

	sfdx force:data:record:update -s User -v "UserRoleId='$newRoleID' FirstName=Buyer" -w "Id=$username"
	...
	then
		sfdx force:data:record:create -s ContactPointAddress -v "AddressType='Shipping' ParentId='$accountID' ActiveFromDate='2020-01-01' ActiveToDate='2040-01-01' City='Vancouver' Country='Canada' Name='Default Shipping' PostalCode='V6B 5A7' State='BC' Street='333 Seymour Street (Shipping)'"
		sfdx force:data:record:create -s ContactPointAddress -v "AddressType='Billing' ParentId='$accountID' ActiveFromDate='2020-01-01' ActiveToDate='2040-01-01' City='Vancouver' Country='Canada' Name='Default Billing' PostalCode='V6B 5A7' State='BC' Street='333 Seymour Street (Billing)'"
		sfdx force:data:record:create -s ContactPointAddress -v "AddressType='Shipping' ParentId='$accountID' ActiveFromDate='2020-01-01' ActiveToDate='2040-01-01' City='Eagan' Country='US' IsDefault='true' Name='Non-Default Shipping' PostalCode='55344' State='MN' Street='415 Mission Street (Shipping)'"
		sfdx force:data:record:create -s ContactPointAddress -v "AddressType='Billing' ParentId='$accountID' ActiveFromDate='2020-01-01' ActiveToDate='2040-01-01' City='Eagan' Country='US' IsDefault='true' Name='Non-Default Billing' PostalCode='55344' State='MN' Street='415 Mission Street (Billing)'"
	else
	...
	#Update price book for Original price
	storeId=$(sfdx force:data:soql:query -q "SELECT Id FROM WebStore WHERE Name='$1' LIMIT 1" -r csv |tail -n +2)
	pricebookId=$(sfdx force:data:soql:query -q "SELECT Id FROM Pricebook2 WHERE Name='BASIC_PRICEBOOK_FROM_QUICKSTART' LIMIT 1" -r csv |tail -n +2)
	echo "StoreId::>>$storeId"
	echo "PriceBook::>>$pricebookId"
	sfdx force:data:record:update -s WebStore -v "StrikethroughPricebookId='$pricebookId'" -w "Id='$storeId'"
