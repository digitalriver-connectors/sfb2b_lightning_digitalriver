...
import attchSourceFailed from "@salesforce/label/c.My_Wallet_DR_Attach_Source_Customer_Failed";
...
import addNewPaymentMthods from "@salesforce/label/c.DR_Add_new_Payment_methods";
...
export default class Drb2b_NewPayments extends NavigationMixin(LightningElement) {
    isNewPayment = true;
    isStepOne = true;
    isStepTwo = false;
    isStepThree = false;
    noBilingAddress = false;
    initilized = false;
        ...
        getBillingContactPointAddress()
            .then((result) => {
                ...
                if (this.contactPointAddresses.length !== 0) {
                    this.noBilingAddress = false;
                } else {
                    this.noBilingAddress = true;
                }
            })
            .catch((error) => {
                this.isLoading = false;
    ...
    fireEventLWC() {
        ...
        let countryIsoCode = countryAndISOCodeMap[this.selectedConPointAddress.Country.toUpperCase()];
    ...
    handleErrorMessage(data) {
        this.isLoading = false;
        this.showToast({ message: data, variant: "error" });
    }
    handleError(data) {
        this.isLoading = false;
        this.showToast({ message: this.label.unableToSavePayment, variant: "error" });
    }
    handleOnReady(data) {
        this.isLoading = false;
        if (typeof data.paymentMethodTypes === "undefined" || data.paymentMethodTypes.length == 0) {
            this.showToast({ message: this.label.emptyDropIn, variant: "error" });
        }
    }
    handleSuccess(data) {
        this.isLoading = false;
        ...
        if (!data.readyForStorage) {
            this.showToast({ message: this.label.myWalletSourceNotSaved, variant: "error" });
            return;
        }
            ...
            .then((result) => {
                ...
                if (resultData.isSuccess) {
                    ...
                    setTimeout(() => {
                        ...
                        componet.dispatchEvent(selectedEvent);
                        // component.isLoading = false;
                    }, 1500);
                } else {
                    this.showToast({ message: this.label.attchSourceFailed, variant: "error" });
                }
            })
            .catch((error) => {
                this.isLoading = false;
