...
export default class Drb2BAddTaxCertificate extends LightningElement {
    taxCertificate = {};
    countryOptions;
    stateOptions;
    country = "US";
    ...
    updateCustomerWithTaxCertificate(drFile) {
        ...
        updateCustomerWithTaxCertificate({ taxCertificate: this.taxCertificate, cartId :  currentCartId})
            .then((result) => {
                if (result.isSuccess) {
                    ...
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: "Success",
                            message: taxCertificateCreatedMsg,
                            variant: "success"
                        })
                    );
    ...
    submitCertificate() {
        if (this.isInputValid() && this.fileData.base64File) {
            ...
            console.log('Test Date', desiredDate);
            let filedata={
            ...
            console.log('testing 181', this.fileData);
            this.dispatchEvent(
                new ShowToastEvent({
                    title: "Error",
                    message: this.labels.selectMissingFileMsg,
                    variant: "error"
                })
            );
        }
    ...
    handleUpload(event) {
        ...
        console.log('file aval', isLessThan10Mb(file));
