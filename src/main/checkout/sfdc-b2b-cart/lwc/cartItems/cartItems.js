...
import sku_no from "@salesforce/label/c.DR_SKU_NO";
import per_price from "@salesforce/label/c.DR_Price_Per_Unit";
import total_price from "@salesforce/label/c.DR_Total_Price";
...
export default class Items extends NavigationMixin(LightningElement) {
    ...
    get displayItems() {
        return this._items.map((item) => {
            ...
            newItem.showNegotiatedPrice = this.showNegotiatedPrice && (newItem.cartItem.totalPrice || "").length > 0;

            // Set whether or not to display original price
            newItem.showOriginalPrice = displayOriginalPrice(this.showNegotiatedPrice, this.showOriginalPrice, newItem.cartItem.totalPrice, newItem.cartItem.totalListPrice);
            // get the label for original price to provide to the aria-label attr for screen readers
            newItem.originalPriceLabel = getLabelForOriginalPrice(this.currencyCode, newItem.cartItem.totalListPrice);
            return newItem;
        });
    }
